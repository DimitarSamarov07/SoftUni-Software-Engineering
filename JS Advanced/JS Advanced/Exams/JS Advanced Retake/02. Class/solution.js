function solveClasses() {
    class Hall {
        ...
        toString() {
            ...
            if (this.events.length > 0) {
                if (this.events.find(x => !!x.performers)) {
                    result += "Events: " + this.events.map(x => x.title).join(", ");
                } else {
    ...
    class MovieTheater extends Hall {
        ...
        toString() {
            return super.toString() + `\n${this.name} is a movie theater with`
                + ` ${this.screenSize} screensize and ${this.capacity} seats capacity.`
        }
    }
    ...
    class ConcertHall extends Hall {
        ...
        toString() {
            ...
            if (this.events.length > 0) {
                let performers = [];
                this.events.forEach(x => performers.push(...x.performers));
...
console.log(concert.hallEvent('The Chromatica BallU', ['LADY GAGA']));
